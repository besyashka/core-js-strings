...
function removeFirstOccurrences(str, value) {
  const changeStr = str.replace(value, '');
...
function removeLastOccurrences(str, value) {
  ...
  let result = '';
  if (lastIndex !== -1) {
    result = str.slice(0, lastIndex) + str.slice(lastIndex + value.length);
    return result;
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  const strMinutes = String(minutes);
  const strSeconds = String(seconds);
  const checkLengthMin =
    strMinutes.length < 2 ? `${'0'}${strMinutes}` : strMinutes;
  const checkLengthSec =
    strSeconds.length < 2 ? `${'0'}${strSeconds}` : strSeconds;
  return checkLengthSec.padStart(5, `${checkLengthMin}${':'}`);
}
...
function countVowels(str) {
  const strMatch = str.match(/[aeiouyAEIOUY]/g);
  const countingVowels = strMatch === null ? 0 : strMatch.length;
...
function reverseWords(str) {
  const flipLetters = str.split('').reverse().join('');
  const flipWords = flipLetters.split(' ').reverse().join(' ');
  return flipWords;
}
...
function encodeToRot13(str) {
  ...
  const encryptedABC = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  const replaceLetters = str.replace(
    /[A-Z]/gi,
    (letter) => encryptedABC[alphabet.indexOf(letter)]
  );
